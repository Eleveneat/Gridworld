	...
	public boolean ASearch() throws IOException{
		...
		while (this.openList.isEmpty() != true && searchedNodesNum <= maxNodesNum) {
			...
			searchedNodesNum++;
				...
				// 记录并显示搜索过程
				pw.println("Searching.....Number of searched nodes:" + this.closeList.size() + "   Current state:" + this.currentJNode.toString());
				System.out.println("Searching.....Number of searched nodes:" + this.closeList.size() + "   Current state:" + this.currentJNode.getEstimatedValue() + this.currentJNode.toString());			

			// (2-3)寻找所有与currentJNode邻接且未曾被访问的节点，将它们按代价估值从小到大排序插入openList中
			followJNodes = this.findFollowJNodes(this.currentJNode);
			
			for (int i = 0; i < followJNodes.size(); i++) {
				estimateValue(followJNodes.elementAt(i));
			}
			
			while (!followJNodes.isEmpty()) {
				JigsawNode tmp = followJNodes.elementAt(0);
				int maxEstimateValue = tmp.getEstimatedValue();
				for (int i = 1; i < followJNodes.size(); i++) {
					if (followJNodes.elementAt(i).getEstimatedValue() < maxEstimateValue) {
						tmp = followJNodes.elementAt(i);
						maxEstimateValue = tmp.getEstimatedValue();
					}
				}
				this.sortedInsertOpenList(tmp);
				followJNodes.removeElement(tmp);
			}
		}
...
//				int otherRealNum = jNode.getNodesState()[realNum];
//				int otherRealX = (otherRealNum - 1) % dimension;
//				int otherRealY = (otherRealNum - 1) / dimension;
//				int otherIndex = realNum;
//				int otherIndexX = (otherIndex - 1) % dimension;
//				int otherIndexY = (otherIndex - 1) / dimension;
//				distance = Math.abs(otherRealX - otherIndexX) + Math.abs(otherRealY - otherIndexY);
//				manhattanDistance += distance;
			}	
		}
